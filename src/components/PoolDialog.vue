...
<template>
  <div class="modal-backdrop" @click.self="emit('close')">
    <section class="quote-modal" role="dialog" aria-modal="true" aria-label="Request a pool quote">
      ...
      <div class="modal-pool"><img :src="asset(pool.shellImage || pool.image)" :alt="pool.name"><div><p class="eyebrow dark">Selected pool</p><h2>{{ pool.name }}</h2><p>{{ pool.size }} <span>•</span> {{ pool.depth }}</p><strong>{{ pool.price !== 'R0' ? `From ${pool.price}` : 'Price on request' }}</strong></div></div>
